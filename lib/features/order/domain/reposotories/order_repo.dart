...
class OrderRepo {
  ...
  Future<ApiResponseModel> getDeliveryOrderStatistics({String? filter}) async {
    try {
      final response = await dioClient!.get('${AppConstants.deliveryOrderStatisticsUri}?token=${sharedPreferences!.get(AppConstants.token)}&filter=${filter ?? DeliveryAnalyticsTimeRangeEnum.allTime.name}');
      return ApiResponseModel.withSuccess(response);
    } catch (e) {
      return ApiResponseModel.withError(ApiErrorHandler.getMessage(e));
    }
